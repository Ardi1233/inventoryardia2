...
namespace Daftar_Pepustakaan
{
    public partial class Dashboard : Form
    {
        ...
        {
            MySqlCommand command = new MySqlCommand("select * from data_barang where concat(id,nama_produk, kode_barang, jenis, satuan, jumlah, harga, foto) like '%" + valueToSearch + "%'", connection);
            ...
            dataGridView1.Columns[1].HeaderText = "nama_produk";
            dataGridView1.Columns[2].HeaderText = "kode_barang";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            byte[] img = ms.ToArray();
            MySqlCommand command = new MySqlCommand("INSERT INTO data_barang(nama_produk, kode_barang, jenis, satuan, jumlah, harga, foto) VALUES (@nama_produk,@kode_barang,@jenis,@satuan,@jumlah,@harga,@foto)", connection);
            command.Parameters.Add("@nama_produk", MySqlDbType.VarChar).Value = textBox1.Text;
            command.Parameters.Add("@kode_barang", MySqlDbType.VarChar).Value = textBox2.Text;
            command.Parameters.Add("@jenis", MySqlDbType.VarChar).Value = textBox3.Text;
            command.Parameters.Add("@satuan", MySqlDbType.VarChar).Value = textBox4.Text;
            command.Parameters.Add("@jumlah", MySqlDbType.VarChar).Value = textBox5.Text;
            command.Parameters.Add("@harga", MySqlDbType.VarChar).Value = textBox6.Text;
        ...
        {
            MySqlCommand command = new MySqlCommand("DELETE FROM data_barang WHERE id=@id", connection);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            pictureBox1.Image.Save(ms, pictureBox1.Image.RawFormat);
            byte[] img = ms.ToArray();
            MySqlCommand command = new MySqlCommand("UPDATE data_barang SET nama_produk=@nama_produk,kode_barang=@kode_barang,jenis=@jenis,satuan=@satuan,jumlah=@jumlah,harga=@harga,foto=@foto WHERE id=@id", connection);
